      ...
      <Link
        ...
        <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 cursor-pointer relative">
              ...
              <div className="w-full h-full bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center">
                <div className="text-center text-muted-foreground">
                  <Text
                    variant="body2"
                    color="neutral"
                    className="mb-2">{variant === "featured" ? `Obrázek: ${item.title}` : item.title}</Text>
                  <Text
                    variant="caption"
                    color="neutral">{item.dimensions}</Text>
                </div>
              </div>
          )}
          ...
          <div className={cn("absolute flex flex-col gap-1", variant === "featured" ? "top-4 left-4" : "top-3 left-3")}>
            {item.featured && (
                <Badge
                  variant="secondary"
                  className={variant === "featured" ? "" : "text-xs"}>
                  Doporučeno
                </Badge>
            )}
            {!item.inStock && (
          ...
          <div className={variant === "featured" ? "flex justify-between items-start" : ""}>
            ...
            {variant === "featured" && (
                <div className="text-right">
                  <Text
                    variant="h5"
                    color="primary">{formatPrice(item.price)}</Text>
                  {item.subcategory && (
                      <Badge
                        variant="outline"
        ...
        <CardContent className={variant === "featured" ? "" : "pt-0"}>
                ...
                <div className="flex justify-between items-center mb-3">
                  <div className="flex flex-col">
                    <Text
                      variant="h5"
                      color="primary"
                      className="font-bold">{formatPrice(item.price)}</Text>
          ...
          <div className={cn("flex relative z-10", variant === "featured" && "gap-2")}>
            <div
              className={cn(variant === "featured" ? "flex-1" : "w-full")}
